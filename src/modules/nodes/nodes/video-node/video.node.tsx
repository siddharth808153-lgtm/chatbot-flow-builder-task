...
const NODE_TYPE = BuilderNode.VIDEO;

export type { VideoNodeData };

type VideoNodeProps = NodeProps<Node<VideoNodeData, typeof NODE_TYPE>>;

export function VideoNode({ id, isConnectable, selected, data }: VideoNodeProps) {
    ...
    return (
        <>
            <div
                ...
                className="w-xs overflow-clip border border-dark-200 rounded-xl bg-dark-300/50 shadow-sm backdrop-blur-xl transition divide-y divide-dark-200 data-[selected=true]:(border-rose-600 ring-1 ring-rose-600/50)"
                onDoubleClick={showNodeProperties}
            >
                <div className="relative bg-dark-300/50">
                    <div className="absolute inset-0">
                        <div className="absolute h-full w-3/5 from-rose-900/20 to-transparent bg-gradient-to-r" />
                    </div>
...
export const metadata: RegisterNodeMetadata<VideoNodeData> = {
    type: NODE_TYPE,
    node: memo(VideoNode),
    detail: {
        icon: "i-mynaui:video",
